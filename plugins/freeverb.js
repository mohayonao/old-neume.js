(function(plugin) {
  ...
  neume.register("freeverb", function(ugen, spec, inputs) {
    ...
    var outlet  = context.createScriptProcessor(neume.PLUGIN_BUF_SIZE, 1, 1);

    var mix  = _.finite(_.defaults(spec.mix, 0.33));
    var room = _.finite(_.defaults(spec.room, 0.5));
    var damp = _.finite(_.defaults(spec.damp, 0.5));

    var freeVerb = new FreeVerb(mix, room, damp);

    outlet.onaudioprocess = function(e) {
      freeVerb.process(
        e.inputBuffer.getChannelData(0),
        e.outputBuffer.getChannelData(0),
        neume.PLUGIN_BUF_SIZE
      );
    };
  ...
  function FreeVerb(mix, room, damp) {
    this.mix  = Math.max(0, Math.min(mix , 1));
    this.room = Math.max(0, Math.min(room, 1));
    this.damp = Math.max(0, Math.min(damp, 1));
  ...
  FreeVerb.prototype.process = function(_in, out, inNumSamples) {
    var ftemp0 = this.mix;
    var ftemp1 = 1 - ftemp0;
    var ftemp5 = 0.7 + (0.28 * this.room);
    var ftemp6 = 0.4 * this.damp;
    var ftemp7 = 1 - ftemp6;
